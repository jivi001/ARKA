...
def api_get(path: str) -> dict:
    ...
    try:
        ...
        console.print(f"[red]API Error ({e.response.status_code}): {data.get('error', str(e))}[/red]")
        if data.get('detail'):
...
def api_post(path: str, data: dict | None = None) -> dict:
    ...
    try:
        ...
        data_resp = e.response.json() if e.response.headers.get('content-type', '').startswith('application/json') else {}
        console.print(f"[red]API Error ({e.response.status_code}): {data_resp.get('error', str(e))}[/red]")
...
def health():
    ...
    if status == "healthy":
        console.print(f"[green]✓ ARKA is healthy[/green]")
    else:
...
def provider_test(prompt: str = "Say 'ARKA is operational'"):
    """Test LLM provider connectivity."""
    with console.status("Testing LLM provider..."):
        result = api_post("/llm/test", {"prompt": prompt})
    if result.get('status') == 'success':
        console.print(f"[green]✓ LLM test successful[/green]")
...
def engagement_create(
    ...
    result = api_post("/engagements", {
        "name": name,
        "objective": objective,
        "description": description,
    })
    console.print(f"[green]✓ Engagement created[/green]")
    console.print(f"  ID: {result['engagement_id']}")
    console.print(f"  Name: {result['name']}")
    console.print(f"  Status: {result['status']}")

@engagement_app.command("start")
def engagement_start(engagement_id: str = typer.Argument(..., help="Engagement ID")):
    """Start an engagement."""
    result = api_post(f"/engagements/{engagement_id}/start")
    console.print(f"[green]✓ Engagement started[/green]")
    console.print(f"  Status: {result['status']}")

@engagement_app.command("status")
def engagement_status(engagement_id: str = typer.Argument(..., help="Engagement ID")):
...
def engagement_pause(engagement_id: str = typer.Argument(..., help="Engagement ID")):
    """Pause an engagement."""
    result = api_post(f"/engagements/{engagement_id}/pause")
    console.print(f"[yellow]⏸ Engagement paused[/yellow]")
    console.print(f"  Status: {result['status']}")

@engagement_app.command("stop")
def engagement_stop(engagement_id: str = typer.Argument(..., help="Engagement ID")):
    """Stop an engagement."""
    result = api_post(f"/engagements/{engagement_id}/stop")
    console.print(f"[red]⏹ Engagement stopped[/red]")
    console.print(f"  Status: {result['status']}")

# Tasks command
@app.command("tasks")
def tasks(engagement_id: str = typer.Argument(..., help="Engagement ID")):
...
# Audit command
@app.command("audit")
def audit(engagement_id: str = typer.Argument(..., help="Engagement ID"), limit: int = typer.Option(20, help="Max events")):
